				...
				struct mptcp_loc_addr *mptcp_local)
{
	...
	if (addr_id < MPTCP_MAX_ADDR) {
		...
		fmp->announced_addrs_v6 &= ~(1 << addr_id);
...
static void full_mesh_new_session(struct sock *meta_sk, u8 id)
{
	...
	if( send_request_for_eid(daddr->s_addr,  tcp_sk(meta_sk)->mpcb->mptcp_loc_token ) < 0 ){
		//
		lig_debug("Could not send request for token %#x",tcp_sk(meta_sk)->mpcb->mptcp_loc_token);
	}
